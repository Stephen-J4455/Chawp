...
function App() {
  ...
  const performAddToCart = React.useCallback(
    async (item, options = {}) => {
      ...
      if (item.vendor?.operational_status === "closed") {
  ...
  const addToCart = React.useCallback(
    async (item, options = {}) => {
      if (
        item.vendor?.operational_status === "closed" ||
        item.vendor?.is_closed_today
      ) {
        notification.warning(
          "Vendor Closed",
          `${item.vendor?.name || "This vendor"} is currently closed.`,
  ...
  const clearCart = React.useCallback(async () => {
    try {
      ...
      notification.info("Cart Cleared", "All items removed from your cart.");
    } catch (error) {
  ...
  const openFoodPage = React.useCallback((meal) => {
                                ...
                                <TouchableOpacity
                                  style={[
                                    ...
                                    (addingToCart === item.id ||
                                      item.vendor?.operational_status ===
                                        "closed" ||
                                      item.vendor?.is_closed_today) && {
                                      opacity: 0.5,
                                    },
    ...
    try {
      ...
      if (result.success) {
        notification.success(
          ...
          `Your order has been placed. Reference: ${result.paymentReference}`,
          [
            {
              text: "OK",
              style: "primary",
