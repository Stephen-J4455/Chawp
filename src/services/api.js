  ...
  try {
    ...
    if (filteredMeals.length > 0) {
      ...
      const { data: vendorHours, error: hoursError } = await supabase
        ...
        .select("vendor_id, is_closed")
        .eq("day_of_week", currentDay)
        .in("vendor_id", vendorIds);

      if (!hoursError && vendorHours) {
        // Create a map of vendor_id to is_closed status
        const vendorClosedMap = {};
        vendorHours.forEach((hour) => {
          vendorClosedMap[hour.vendor_id] = hour.is_closed;
        });

        // Add is_closed status to each meal's vendor
        filteredMeals.forEach((meal) => {
          if (meal.vendor && meal.vendor_id in vendorClosedMap) {
            meal.vendor.is_closed_today = vendorClosedMap[meal.vendor_id];
          } else {
